...
Public Class TiposAssistencias_Implem
...
#Region " Métodos "
    ...
    Public Function VerificaExistenciaAssistencia() As Integer
        ...
        Try
            ...
            wComando.CommandText = "SELECT * FROM Ass_TipoAssistencia WHERE Codigo='" & mCodigo & "'" & _
    ...
    Public Function BuscarAssistencia() As Integer

        Dim wTmp As Integer
        If mCodigo = "" Then
            VariaveisGlobais.gUltimoSQL = ""
            VariaveisGlobais.gDescricaoUltimoErro = "Parâmetro incorreto para busca!"
            VariaveisGlobais.gStackTraceErro = "CadastroTiposAssistencias_Implem->BuscarAssistencia()"
            Return -1
        End If

        wTmp = VerificaExistenciaAssistencia()
        If wTmp <> 0 Then Return wTmp
        Dim wControle As Integer
        Try
            wControle = objVariaveisGlobais.VerificaConexaoBD(VariaveisGlobais.gConexao, VariaveisGlobais.gConnectionString)
            If wControle <> 0 Then Return wControle
            wComando.CommandText = "SELECT * FROM Ass_TipoAssistencia WHERE Codigo='" & mCodigo & "'"
            wDataReader = wComando.ExecuteReader
            If wDataReader.Read() Then
    ...
    Public Function IncluirAssistencia() As Integer
        Try
            ...
            wComando.CommandText = " SELECT isnull(max(cast(Codigo as integer)),0)+1 Codigo " & _
                                " FROM Ass_TipoAssistencia WHERE isnumeric(Codigo)<>0 "
            wDataReader = wComando.ExecuteReader
            If wDataReader.Read() Then
                mCodigo = wDataReader("Codigo")
            Else
            ...
            wTransacao = VariaveisGlobais.gConexao.BeginTransaction(IsolationLevel.ReadCommitted, "IncluirAssistencia")
            wComando.Transaction = wTransacao

            Try
                wComando.CommandText = " INSERT INTO ASS_TipoAssistencia (Codigo,  " & _
                        "                Descricao,                       " & _
                ...
                wTransacao.Commit()
            Catch ex As Exception
                wTransacao.Rollback("IncluirAssistencia")
                Throw ex
            End Try
    ...
    Public Function AlterarAssistencia() As Integer
        ...
        If mCodigo = "" Then
            ...
            VariaveisGlobais.gStackTraceErro = "TiposAssistencias_Implem->AlterarAssistencia()"
            Return -1
        End If
        ...
        Try
            ...
            Dim wTransacao As SqlTransaction
            wTransacao = VariaveisGlobais.gConexao.BeginTransaction(IsolationLevel.ReadCommitted, "AlterarAssistencia")
            wComando.Transaction = wTransacao

            Try
                If Len(wSet) > 0 Then
                    wComando.CommandText = _
                           " UPDATE Ass_TipoAssistencia SET " & wSet & " WHERE " & _
                      "  Codigo='" & mCodigo & "'"

                    wComando.ExecuteNonQuery()
                    Dim Log As String = "C_LOG"
                    Dim Tab As String = "Ass_TipoAssistencia"
                ...
                End If
            Catch ex As Exception
                wTransacao.Rollback("AlterarAssistencia")
                Throw ex
            End Try
    ...
    Public Function VerificaDependenciaAssistencia(ByVal pCodigo As String) As Integer
        Dim wControle As Integer
        Try
            wControle = objVariaveisGlobais.VerificaConexaoBD(VariaveisGlobais.gConexao, VariaveisGlobais.gConnectionString)
            If wControle <> 0 Then Return wControle
            wComando.CommandText = "SELECT * FROM Ass_Movimento " & _
                                   " WHERE TipoAssistencia ='" & pCodigo & "'" & _
                                   " AND isnull(Data_Hora_Exclusao,'') = '' "

            wDataReader = wComando.ExecuteReader
            If wDataReader.Read() Then
                wDataReader.Close()
                VariaveisGlobais.gDescricaoUltimoErro = " Serviço é usado em Movimentos!"
                Return 1
            End If
            wDataReader.Close()
        Catch ex As Exception
    ...
    Public Function ExcluirAssistencia() As Integer
        ...
        Try
            ...
            If mCodigo = "" Then
                ...
                VariaveisGlobais.gStackTraceErro = "Assistencias_Implem->ExcluirAssistencia()"
                Return -1
            End If
            ...
            wControle = VerificaDependenciaAssistencia(mCodigo)
            If wControle <> 0 Then Return wControle

            Dim wTransacao As SqlTransaction
            wTransacao = VariaveisGlobais.gConexao.BeginTransaction(IsolationLevel.ReadCommitted, "ExcluirAssistencia")
            wComando.Transaction = wTransacao

            Try
                wComando.CommandText = " UPDATE Ass_TipoAssistencia SET " & _
                ...
                wTransacao.Commit()
            Catch ex As Exception
                wTransacao.Rollback("ExcluirAssistencia")
            End Try
    ...
    Public Function MostraTabelaTiposAssistencias(ByRef pObjTabela As System.Windows.Forms.DataGridView, _
