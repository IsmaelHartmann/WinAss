...
Public Class Animais_Implem
    Inherits Fortaleza.Business.BO

#Region " Variáveis internas "

    Private mCodigo As String
    Private mNomeAnimal As String
...
#Region " Métodos "
    ...
    Public Function VerificaExistenciaAnimal() As Integer
        ...
        Try
            ...
            wComando.CommandText = "SELECT * FROM Ass_Animais WHERE Codigo='" & mCodigo & "'" & _
    ...
    Public Function VerificaDependenciaAnimal(ByVal pCodigo As String) As Integer
        Dim wControle As Integer
        Try
            wControle = objVariaveisGlobais.VerificaConexaoBD(VariaveisGlobais.gConexao, VariaveisGlobais.gConnectionString)
            If wControle <> 0 Then Return wControle
            wComando.CommandText = "SELECT * FROM Ass_Movimento WHERE Animal = '" & mCodigo & "'" & _
                             " AND isnull(Data_Hora_Exclusao,'') = '' "
    ...
    Public Function BuscarAnimal() As Integer

        Dim wTmp As Integer
        If mCodigo = "" Then
            VariaveisGlobais.gUltimoSQL = ""
            VariaveisGlobais.gDescricaoUltimoErro = "Parâmetro incorreto para busca!"
            VariaveisGlobais.gStackTraceErro = "CadastroAnimais_Implem->BuscarAnimal()"
            Return -1
        End If

        wTmp = VerificaExistenciaAnimal()
        If wTmp <> 0 Then Return wTmp
        Dim wControle As Integer
        Try
            wControle = objVariaveisGlobais.VerificaConexaoBD(VariaveisGlobais.gConexao, VariaveisGlobais.gConnectionString)
            If wControle <> 0 Then Return wControle
            wComando.CommandText = "SELECT * FROM Ass_Animais WHERE Codigo='" & mCodigo & "'"
            wDataReader = wComando.ExecuteReader
            If wDataReader.Read() Then
                mCodigo = wDataReader("Codigo")
                mNomeAnimal = IIf(IsDBNull(wDataReader("Nome")), "", wDataReader("Nome"))
    ...
    Public Function IncluirAnimal() As Integer
        Try
            ...
            wComando.CommandText = " SELECT isnull(max(cast(Codigo as integer)),0)+1 Codigo " & _
                                " FROM Ass_Animais WHERE isnumeric(Codigo)<>0 "
            wDataReader = wComando.ExecuteReader
            If wDataReader.Read() Then
                mCodigo = wDataReader("Codigo")
            Else
            ...
            wTransacao = VariaveisGlobais.gConexao.BeginTransaction(IsolationLevel.ReadCommitted, "IncluirAnimal")
            wComando.Transaction = wTransacao

            Try
                wComando.CommandText = " INSERT INTO Ass_Animais (Codigo,  " & _
                        "                Nome,                       " & _
                ...
                wTransacao.Commit()
            Catch ex As Exception
                wTransacao.Rollback("IncluirAnimal")
                Throw ex
            End Try
    ...
    Public Function AlterarAnimal() As Integer
        ...
        If mCodigo = "" Then
            ...
            VariaveisGlobais.gStackTraceErro = "Animais_Implem->AlterarAnimal()"
            Return -1
        End If
        ...
        Try
            ...
            If mNomeAnimal <> wDataReader("Nome") Then
                wSet = wSet & ", Nome = '" & mNomeAnimal & "'"
            ...
            Dim wTransacao As SqlTransaction
            wTransacao = VariaveisGlobais.gConexao.BeginTransaction(IsolationLevel.ReadCommitted, "AlterarAnimal")
            wComando.Transaction = wTransacao

            Try
                If Len(wSet) > 0 Then
                    wComando.CommandText = _
                           " UPDATE Ass_Animais SET " & wSet & " WHERE " & _
                      "  Codigo='" & mCodigo & "'"

                    wComando.ExecuteNonQuery()
                    Dim Log As String = "C_LOG"
                    Dim Tab As String = "C_Animals"
                ...
                End If
            Catch ex As Exception
                wTransacao.Rollback("AlterarAnimal")
                Throw ex
            End Try
    ...
    Public Function ExcluirAnimal() As Integer
        ...
        Try
            ...
            If mCodigo = "" Then
                ...
                VariaveisGlobais.gStackTraceErro = "Animals_Implem->ExcluirAnimal()"
                Return -1
            End If
            ...
            wControle = VerificaDependenciaAnimal(mCodigo)
            If wControle <> 0 Then Return wControle

            Dim wTransacao As SqlTransaction
            wTransacao = VariaveisGlobais.gConexao.BeginTransaction(IsolationLevel.ReadCommitted, "ExcluirAnimal")
            wComando.Transaction = wTransacao

            Try
                wComando.CommandText = " UPDATE Ass_Animais SET " & _
                ...
                wTransacao.Commit()
            Catch ex As Exception
                wTransacao.Rollback("ExcluirAnimal")
            End Try
    ...
    Public Function MostraTabelaAnimais(ByRef pObjTabela As System.Windows.Forms.DataGridView, _
