...
#Region " Windows Form Designer generated code "
    ...
    Friend WithEvents dtgEmpresas As System.Windows.Forms.DataGridView
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.gpDados.Text = "Informações da Empresa"
        '
        'btConfirmaSelecao
        '
        ...
        'tpConsulta
        '
        Me.tpConsulta.Controls.Add(Me.dtgEmpresas)
        ...
        Me.dtgEmpresas.AllowUserToDeleteRows = False
        ...
        Me.dtgEmpresas.ColumnHeadersBorderStyle = System.Windows.Forms.DataGridViewHeaderBorderStyle.None
        ...
        Me.dtgEmpresas.ColumnHeadersDefaultCellStyle = DataGridViewCellStyle1
        Me.dtgEmpresas.ColumnHeadersHeightSizeMode = System.Windows.Forms.DataGridViewColumnHeadersHeightSizeMode.AutoSize
        Me.dtgEmpresas.Columns.AddRange(New System.Windows.Forms.DataGridViewColumn() {Me.Cm})
        Me.dtgEmpresas.Location = New System.Drawing.Point(7, 87)
        Me.dtgEmpresas.Name = "dtgEmpresas"
        Me.dtgEmpresas.RowHeadersBorderStyle = System.Windows.Forms.DataGridViewHeaderBorderStyle.None
        ...
        Me.dtgEmpresas.RowHeadersDefaultCellStyle = DataGridViewCellStyle2
        Me.dtgEmpresas.Size = New System.Drawing.Size(744, 335)
        Me.dtgEmpresas.TabIndex = 1
        '
        'Cm
        '
        Me.Cm.HeaderText = "Cm"
...
#End Region
    ...
    Public objEmpresas_Implem As Empresas_Implem = EmpresasBOFactory.GetFactory().GetInstance()
...
#Region " Inicializações "
    ...
    Private Sub Empresas_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        autoriza = VerificaPrograma("Empresas", VariaveisGlobais.gSetorUsuario, _
        ...
        Try
            ...
            If vSelecao <> "" Then
                ...
                If dtgEmpresas.Rows.Count = 1 Then
    ...
    Private Sub CadastroEmpresas_Closing(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles MyBase.Closing
        objEmpresas_Implem = Nothing
        LimpaErros()
    End Sub

    Private Sub Empresas_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Protected Overrides Sub OnKeyDown(ByVal e As System.Windows.Forms.KeyEventArgs)
        If e.KeyCode = Keys.F1 Then
            objLeituras.VerC_LOG(VariaveisGlobais.gBD, "C_LOG", "C_Empresas", txtCodigo.Text)
        End If
        ...
        End If
        If e.KeyCode = Keys.Enter And dtgEmpresas.Focused Then
            TelaAlteracaoEmpresas()
...
#Region " Adicionar/Alterar /Excluir Empresa "
    ...
    Public Function GuardaDadosEmpresas() As Integer
        Try
            objEmpresas_Implem.Codigo = Replace(txtCodigo.Text, "'", "''")
            objEmpresas_Implem.Nome = Replace(txtNome.Text, "'", "''")

            objEmpresas_Implem.Codigo_Usuario_Cadastro = cbUsuarioCadastro.SelectedItem.value
    ...
    Public Sub TelaAlteracaoEmpresas()
        Try
            Dim wControle As Integer
            LimpaErros()
            ...
            txtCodigo.Text = objEmpresas_Implem.Codigo
            txtNome.Text = objEmpresas_Implem.Nome

            txtDataCadastro.Text = ConverteDataHoraParaString(objEmpresas_Implem.Data_Hora_Cadastro)
            objVariaveisGlobais.SetaComboBox(cbUsuarioCadastro, objEmpresas_Implem.Codigo_Usuario_Cadastro)
    ...
    Public Sub AdicionarEmpresas()
        Try
            ...
            GuardaDadosEmpresas()
            wControle = objEmpresas_Implem.IncluirEmpresa()
            If wControle = 1 Then
                MessageBox.Show("Já existe um Empresa cadastrado com este Codigo!", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
            If wControle <> 0 Then
                objVariaveisGlobais.MostraErro()
                Exit Sub
            End If
            MessageBox.Show("Empresa incluído com sucesso! Numero: " & _
                        objEmpresas_Implem.Codigo, "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Public Sub Alterar()
        ...
        Try
            ...
            wControle = GuardaDadosEmpresas()
            If wControle <> 0 Then
                objVariaveisGlobais.MostraErro()
                Exit Sub
            End If

            wControle = objEmpresas_Implem.AlterarEmpresa()
            If wControle <> 0 Then
                objVariaveisGlobais.MostraErro()
                Exit Sub
            End If
            MessageBox.Show("Dados da Empresa modificados com sucesso!", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub Excluir()
        ...
        wMensagem = MessageBox.Show("TODOS Dados relacionados a essa Empresa serão permanentemente excluídos!" & vbCrLf & " Você deseja excluir esta Empresa? ", "Confirmação", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2)
        If (wMensagem = Windows.Forms.DialogResult.Yes) Then
            objEmpresas_Implem.Codigo = txtCodigo.Text
            wControle = objEmpresas_Implem.ExcluirEmpresa()
            If wControle <> 0 Then
                objVariaveisGlobais.MostraErro()
                Exit Sub
            End If
...
#Region " Atualização do DataGrid e do Form "
    ...
    Private Sub AtualizaEmpresas()
        ...
        wControle = objEmpresas_Implem.MostraTabelaEmpresas(dtgEmpresas, wSQL)
    ...
    Private Sub PreencheCampoPesquisa()
        If cbCampo.Items.Count = 0 Then
            ...
            For i As Integer = 0 To dtgEmpresas.Columns.Count - 1
                If dtgEmpresas.Columns(i).ValueType.ToString = GetType(String).ToString AndAlso dtgEmpresas.Columns(i).Visible = True Then
                    c = New System.Web.UI.WebControls.ListItem
                    c.Value = dtgEmpresas.Columns(i).HeaderText
                    c.Text = dtgEmpresas.Columns(i).HeaderText
                    cbCampo.Items.Add(c)
                End If
            Next
    ...
    Private Sub cbEmpresasPesquisaEstado_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbCampo.SelectedIndexChanged
        FazFiltro()
    End Sub
    ...
    Function FazFiltro()
        If Not IsNothing(dtgEmpresas.DataSource) Then
            ...
            For i As Integer = 0 To dtgEmpresas.Columns.Count - 1
                If dtgEmpresas.Columns(i).Name = cbCampo.Text Then
                    wValida = True
                    Exit For
                End If
            Next
            ...
            wDataTable = objEmpresas_Implem.wDataSet.Tables(0)
            Dim wDataView = New DataView(wDataTable, _
                     wPesquisa, _
                     "", DataViewRowState.CurrentRows)
            dtgEmpresas.DataSource = wDataView
        End If
        Return True
    End Function

    Private Sub dtgEmpresas_MouseMove(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles dtgEmpresas.MouseMove
        Try
            Dim hti As System.Windows.Forms.DataGridView.HitTestInfo
            hti = dtgEmpresas.HitTest(e.X, e.Y)
            ...
            If hti.Type = System.Windows.Forms.DataGrid.HitTestType.Cell Then
                ...
                End If
                If hti.ColumnIndex = 2 Then
                    ToolTip1.SetToolTip(dtgEmpresas, "")
                End If
            Else
                Me.Cursor = System.Windows.Forms.Cursors.Default
                Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
    ...
    Private Sub dtgEmpresas_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles dtgEmpresas.MouseUp
        Try
            Dim hti As DataGridView.HitTestInfo = dtgEmpresas.HitTest(e.X, e.Y)
    ...
    Private Sub dtgEmpresas_CurrentCellChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dtgEmpresas.CurrentCellChanged
        Try
            Dim wControle As Integer
            If IsNothing(dtgEmpresas.DataSource) Or IsNothing(dtgEmpresas.CurrentCell) Then Exit Sub

            objEmpresas_Implem.Codigo = dtgEmpresas.Item(0, dtgEmpresas.CurrentCell().RowIndex).Value
            wControle = objEmpresas_Implem.BuscarEmpresa
            If wControle <> 0 Then
                objVariaveisGlobais.MostraErro()
                Exit Sub
            End If
            dtgEmpresas.Rows(dtgEmpresas.CurrentCell().RowIndex).Selected = True
            dtgEmpresas.Focus()
        Catch ex As Exception
            VariaveisGlobais.gUltimoSQL = ""
            VariaveisGlobais.gDescricaoUltimoErro = ex.Message
...
#Region " Valida da tela "
    ...
    Private Sub txtCodigo_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtCodigo.Validating
        ...
        Dim wMsgErro As String = "Digite o Codigo do Empresa!"
    ...
    Private Sub txtNome_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtNome.Validating
        Dim wMsgErro As String = "Digite o Nome do Empresa!"
...
#End Region
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) _
        ...
        If mpreview = False Then
            ...
            dados = 0
        End If
        TotalRecords = dtgEmpresas.RowCount
    ...
    Private Sub PrintDocument2_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        For i As Integer = ContadorRegistro To TotalRecords - 1
            ...
            Codigo = dtgEmpresas.Item(0, i).Value
            Nome = dtgEmpresas.Item(1, i).Value
    ...
    Private Sub MontaCabecalho(ByVal e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        e.Graphics.DrawString("Relatório de Empresas", printFontBB, sdb, 390, 55, dFac)
        ...
        e.Graphics.DrawString("Nome do Empresa", printFontB, sdb, 80, 120)
